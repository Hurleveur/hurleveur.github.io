...
function renderToSvg(data, opts = {}, ctx) {
  ...
  const renderedElements = elements.map((el) => linkElement(renderElement(el, data, ctx), ctx?.resolvedLinks?.[el.id])).filter(Boolean);
  const parts = [
...
function linkElement(svg3, href) {
  if (!svg3 || !href) return svg3;
  const external = /^https?:\/\//.test(href) ? ' target="_blank" rel="noopener noreferrer"' : "";
  return `<a href="${escapeAttr(href)}" class="excalidraw-link"${external}>${svg3}</a>`;
}
...
// src/links.ts
function findPage(target, allFiles) {
  const name = (target.split(/[#|]/)[0] ?? "").trim();
  const wanted = slugifyFilePath(name);
  const wantedIndex = wanted.endsWith("/index") ? wanted : `${wanted}/index`;
  const page = allFiles.find((f5) => f5.slug === wanted || f5.slug === wantedIndex) ?? allFiles.find((f5) => f5.slug?.endsWith(`/${wanted}`) || f5.slug?.endsWith(`/${wantedIndex}`));
  return { page, wanted };
}
function drawingLinks(data, published) {
  const links = /* @__PURE__ */ new Set();
  for (const el of data.elements) {
    const source = `${el.link ?? ""} ${el.rawText ?? ""}`;
    for (const [, target] of source.matchAll(/\[\[([^\]]+)\]\]/g)) {
      const { page } = findPage(target, published);
      if (page?.slug) links.add(simplifySlug(page.slug));
    }
  }
  return [...links];
}
...
function resolveLinks(data, currentSlug, allFiles) {
  const result = {};
  for (const el of data.elements) {
    ...
    if (wiki) {
      ...
      if (page?.slug) result[el.id] = resolveRelative(currentSlug, page.slug);
    } else if (url) {
      result[el.id] = url[0];
    }
  }
...
function renderOverlay(overlay, vb) {
  const label = overlay.link.replace(/^\[\[/, "").replace(/\]\]$/, "").replace(/^https?:\/\//, "");
  const truncatedLabel = label.length > 50 ? label.slice(0, 47) + "..." : label;
  const box = `left:${(overlay.x + vb.offsetX) / vb.width * 100}%;top:${(overlay.y + vb.offsetY) / vb.height * 100}%;width:${overlay.width}px;height:${overlay.height}px`;
        ...
        children: /* @__PURE__ */ u4(
          "div",
          {
            ...
            dangerouslySetInnerHTML: {
              __html: overlay.resolved ? `<div class="excalidraw-embed-body">${overlay.resolved.html}</div>` : `<span class="excalidraw-embed-missing">Note not found</span>`
            }
          }
        )
      }
  ...
  return /* @__PURE__ */ u4("div", { class: "excalidraw-overlay excalidraw-embed-url", style: box, "data-overlay-id": overlay.id, children: [
...
var ExcalidrawBody_default = ((userOpts) => {
  const Component = (props) => {
    ...
    const renderCtx = {
      ...
      resolvedLinks: allFiles ? resolveLinks(data, currentSlug, allFiles) : void 0
    };
    ...
    return /* @__PURE__ */ u4(k3, { children: [
      /* @__PURE__ */ u4("div", { class: "excalidraw-view", children: [
        /* @__PURE__ */ u4(
          "div",
          {
            ...
            "aria-label": label,
            children: [
              /* @__PURE__ */ u4("div", { class: "excalidraw-svg", dangerouslySetInnerHTML: { __html: result.svg } }),
              /* @__PURE__ */ u4("div", { class: "excalidraw-overlays", children: result.overlays.map((o4) => renderOverlay(o4, vb)) })
            ]
          }
        ),
        /* @__PURE__ */ u4("button", { type: "button", class: "excalidraw-expand", "aria-label": `Open ${label} full screen`, children: "\u2922" })
      ] }),
      /* @__PURE__ */ u4("dialog", { class: "excalidraw-dialog", "aria-label": label, children: [
        /* @__PURE__ */ u4("button", { class: "excalidraw-dialog-close", type: "button", "aria-label": "Close", children: "\u2715" }),
        /* @__PURE__ */ u4("article", { class: "excalidraw-page", children: [
          ...
          /* @__PURE__ */ u4("div", { class: "excalidraw-container" })
        ] })
      ] }),
      /* @__PURE__ */ u4("dialog", { class: "excalidraw-note-dialog", children: [
        /* @__PURE__ */ u4("button", { class: "excalidraw-dialog-close", type: "button", "aria-label": "Close", children: "\u2715" }),
        /* @__PURE__ */ u4("h2", { class: "excalidraw-note-title" }),
        /* @__PURE__ */ u4("div", { class: "excalidraw-note-body" })
      ] }),
...
var ExcalidrawPage = (opts) => ({
  ...
  generate({ ctx, content }) {
    const published = content.map((c4) => c4[1].data);
    const allowlist = loadPublishAllowlist();
    const excalidrawFiles = ctx.allFiles.filter(
      (fp) => (fp.endsWith(".excalidraw.md") || fp.endsWith(".excalidraw")) && allowlist.includes(fp)
    );
    ...
    for (const filePath of excalidrawFiles) {
      ...
      let content2;
      try {
        content2 = readFileSync(fullPath, "utf-8");
      } catch {
        continue;
      }
      const data = parseExcalidraw(content2, filePath);
      if (!data) continue;

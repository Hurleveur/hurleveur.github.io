...
// same palette + hash as vaultbrain.js folderColor — explorer folder identity
...
function folderColor(folder) {
  ...
  if (FOLDER_COLORS[key]) return FOLDER_COLORS[key];
  let h = 0;
...
async function buildFileTrie(dataFns) {
  try {
    ...
    // page-like assets (html/pdf) live outside the content index; the Assets
    // emitter publishes their listing in static/assetIndex.json
    try {
      const res = await fetch(resolveBasePath("static/assetIndex.json"));
      if (res.ok) {
        for (const asset of await res.json()) {
          trie.add({ ...asset, asset: true });
        }
      }
    } catch (e) {
      console.warn("[Explorer] No asset index:", e);
    }

    // LOCI PATCH: notes that name a folder in their `categories` frontmatter
    // nest under that folder too, wherever they physically live (Categories
    // emitter -> static/categoryIndex.json). The trie places a node by its
    // slug, so the entry gets the category path while `href` keeps the real
    // one — see the file-link branch of renderNode below.
    try {
      const res = await fetch(resolveBasePath("static/categoryIndex.json"));
      if (res.ok) {
        for (const [folder, members] of Object.entries(await res.json())) {
          for (const member of members) {
            const name = member.slug.split("/").pop();
            trie.add({ ...member, slug: folder + "/" + name, href: member.slug });
          }
        }
      }
    } catch (e) {
      console.warn("[Explorer] No category index:", e);
    }
...
function renderTree(node, container, currentSlug, folderBehavior, savedState, pathPrefix = "") {
  ...
  if (node.isFolder) {
    ...
    // top-level folder sets its graph color; children inherit the custom property
    if (pathPrefix === "") {
      const li = clone.querySelector("li");
      if (li) li.style.setProperty("--fc", folderColor(node.slugSegment));
    }
    ...
    if (link) {
      // LOCI PATCH: a category member sits under its category folder but the
      // page itself lives at `href` — link and highlight the real note
      const target = node.data.href ?? node.data.slug;
      link.href = resolveBasePath(target);
      link.textContent = node.displayName || node.slugSegment;
      // guest = listed here by category, lives elsewhere; custom.scss draws the ↗
      if (node.data.origin) {
        link.classList.add("category-guest");
        link.title = "in " + node.data.origin;
      }
      // raw html/pdf assets aren't quartz pages — full page load, not SPA swap
      if (node.data.asset) link.dataset.routerIgnore = "";
...
async function handleNavOrRender(e) {
  ...
  try {
    ...
    for (const explorer of document.getElementsByClassName("explorer")) {
      ...
      // collapse BEFORE un-hiding the toggle: checkVisibility() below forces a
      // style flush, and only adding .collapsed after it makes the drawer's
      // 200ms transition run open->closed in full view on slow devices

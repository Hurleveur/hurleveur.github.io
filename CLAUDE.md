...
- **Deploy failures are silent** — the run just doesn't push, and nothing surfaces it. `journalctl --user -u loci-deploy.service | grep -E 'Finished|Failed|gh-pages ->'` is the honest history; `git log origin/gh-pages -1` after a plain `git fetch origin gh-pages` is not, that leaves the tracking ref stale. Use `git ls-remote origin gh-pages`.
